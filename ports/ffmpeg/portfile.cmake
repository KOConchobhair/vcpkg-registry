# ROC: begin -- Jetson nvmpi support. Everything ROC adds to this port is inside "ROC:" blocks so
# that resyncing with a newer upstream port is a diff rather than a merge. See ../README.md.
#
# The nvmpi patch is self-contained: it creates every libavcodec/libavfilter source it needs and
# registers the codecs and the VIC filter, so no post-extract patch script is involved. It is
# carried by the jetson-nvmpi port, which pins the upstream revision it came from.
set(ROC_NVMPI_PATCH "")
if("nvmpi" IN_LIST FEATURES)
    set(ROC_NVMPI_PATCH "${CURRENT_INSTALLED_DIR}/share/jetson-nvmpi/patches/ffmpeg7.0_nvmpi.patch")
    if(NOT EXISTS "${ROC_NVMPI_PATCH}")
        message(FATAL_ERROR "jetson-nvmpi did not install an nvmpi patch for FFmpeg ${VERSION}")
    endif()
endif()
# ROC: end
...
vcpkg_from_github(
    ...
    PATCHES
        ...
        ${ROC_NVMPI_PATCH} # ROC: empty unless the nvmpi feature is on
)
...
# ROC: begin -- CUDA filters (scale_cuda, overlay_cuda, ...).
# Deliberately cuda-llvm and not cuda-nvcc. Both satisfy scale_cuda_filter_deps_any, but
# cuda_nvcc sits on FFmpeg's HWACCEL_LIBRARY_NONFREE_LIST, so it forces --enable-nonfree and
# makes the result unredistributable. cuda-llvm compiles the same kernels with clang and stays
# redistributable, which is what the SDK requires. clang must be present on the build host;
# vcpkg does not supply it.
if("cuda-llvm" IN_LIST FEATURES)
    if(NOT "nvcodec" IN_LIST FEATURES)
        message(FATAL_ERROR "The cuda-llvm feature requires the nvcodec feature (ffnvcodec headers).")
    endif()
    set(OPTIONS "${OPTIONS} --enable-cuda-llvm")
else()
    set(OPTIONS "${OPTIONS} --disable-cuda-llvm")
endif()

# ROC: Jetson hardware encode/decode. libnvmpi is linked against stub libraries so it can be
# built off-device; -rpath-link lets the configure probe and the link step resolve those stubbed
# NEEDED entries. It is a link-time search path only, so the stubs can never shadow the real
# Tegra libraries at run time.
if("nvmpi" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-nvmpi")
    set(OPTIONS "${OPTIONS} --extra-ldflags=-Wl,-rpath-link,${CURRENT_INSTALLED_DIR}/share/jetson-nvmpi/stubs")
endif()
# ROC: end
